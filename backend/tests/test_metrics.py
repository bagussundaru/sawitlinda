...
class TestPencocokan:
    def test_deteksi_sempurna(self):
        gt = [GroundTruth((0, 0, 10, 10), "Sehat", "a")]
        pred = [Prediction((0, 0, 10, 10), "Sehat", 0.9, "a")]
    ...
    def test_satu_ground_truth_hanya_dipasangkan_sekali(self):
        ...
        gt = [GroundTruth((0, 0, 10, 10), "Sehat", "a")]
        pred = [
            Prediction((0, 0, 10, 10), "Sehat", 0.9, "a"),
            Prediction((1, 1, 10, 10), "Sehat", 0.8, "a"),
        ]
    ...
    def test_iou_di_bawah_ambang_tidak_dicocokkan(self):
        ...
        pred = [Prediction((8, 0, 10, 10), "Sehat", 0.9, "a")]

        hasil = evaluate(pred, gt)

        assert hasil.per_class[0].true_positive == 0
        assert hasil.confusion[BACKGROUND]["Sehat"] == 1
        assert hasil.confusion["Sehat"][MISSED] == 1

    def test_pencocokan_tidak_melintasi_citra(self):
        gt = [GroundTruth((0, 0, 10, 10), "Sehat", "a")]
        pred = [Prediction((0, 0, 10, 10), "Sehat", 0.9, "b")]
    ...
    def test_kotak_bertumpang_dipasangkan_ke_yang_iou_terbesar(self):
        gt = [
            GroundTruth((0, 0, 10, 10), "Sehat", "a"),
            GroundTruth((6, 0, 10, 10), "Menguning", "a"),
        ]
        pred = [Prediction((6, 0, 10, 10), "Menguning", 0.9, "a")]

        hasil = evaluate(pred, gt)

        menguning = next(m for m in hasil.per_class if m.label == "Menguning")
        assert menguning.true_positive == 1


class TestSalahKelas:
    def test_kotak_benar_kelas_salah_tercatat_di_confusion(self):
        gt = [GroundTruth((0, 0, 10, 10), "Menguning", "a")]
        pred = [Prediction((0, 0, 10, 10), "Kerdil", 0.9, "a")]

        hasil = evaluate(pred, gt)

        # Terlihat sebagai kekeliruan kelas, bukan sebagai objek yang terlewat.
        assert hasil.confusion["Menguning"]["Kerdil"] == 1
        assert hasil.confusion["Menguning"][MISSED] == 0

        menguning = next(m for m in hasil.per_class if m.label == "Menguning")
        kerdil = next(m for m in hasil.per_class if m.label == "Kerdil")
...
class TestAveragePrecision:
    def test_ap_satu_saat_semua_benar(self):
        gt = [GroundTruth((i * 100, 0, 10, 10), "Sehat", "a") for i in range(4)]
        pred = [
            Prediction((i * 100, 0, 10, 10), "Sehat", 0.9 - i * 0.1, "a") for i in range(4)
        ]
    ...
    def test_urutan_confidence_mempengaruhi_ap(self):
        ...
        salah_dulu = evaluate(
            [
                Prediction((500, 500, 10, 10), "Sehat", 0.95, "a"),
                Prediction((0, 0, 10, 10), "Sehat", 0.5, "a"),
            ],
            gt,
        )
        benar_dulu = evaluate(
            [
                Prediction((0, 0, 10, 10), "Sehat", 0.95, "a"),
                Prediction((500, 500, 10, 10), "Sehat", 0.5, "a"),
            ],
            gt,
        )
    ...
    def test_ap_dihitung_tangan_untuk_kasus_campuran(self):
        ...
        gt = [
            GroundTruth((0, 0, 10, 10), "Sehat", "a"),
            GroundTruth((100, 0, 10, 10), "Sehat", "a"),
        ]
        pred = [
            Prediction((0, 0, 10, 10), "Sehat", 0.9, "a"),
            Prediction((500, 500, 10, 10), "Sehat", 0.8, "a"),
            Prediction((100, 0, 10, 10), "Sehat", 0.7, "a"),
        ]
...
class TestAgregat:
    def test_map_hanya_merata_ratakan_kelas_yang_punya_ground_truth(self):
        ...
        pred = [
            Prediction((0, 0, 10, 10), "Sehat", 0.9, "a"),
            Prediction((900, 900, 10, 10), "Kerdil", 0.4, "a"),
        ]

        hasil = evaluate(pred, gt)

        assert hasil.map50 == pytest.approx(1.0)
        assert any(m.label == "Kerdil" and m.support == 0 for m in hasil.per_class)

    def test_micro_f1_konsisten_dengan_presisi_dan_recall(self):
        gt = [GroundTruth((i * 100, 0, 10, 10), "Sehat", "a") for i in range(4)]
        pred = [
            Prediction((0, 0, 10, 10), "Sehat", 0.9, "a"),
            Prediction((100, 0, 10, 10), "Sehat", 0.8, "a"),
            Prediction((900, 900, 10, 10), "Sehat", 0.7, "a"),
        ]
    ...
    def test_ambang_iou_dapat_diatur(self):
        gt = [GroundTruth((0, 0, 10, 10), "Sehat", "a")]
        pred = [Prediction((3, 0, 10, 10), "Sehat", 0.9, "a")]  # IoU = 70/130 = 0,538
